# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  chezmoi-vim = {
    pname = "chezmoi-vim";
    version = "9b9f0479856f5ce61daa1d5889ff379b18a109a5";
    src = fetchFromGitHub ({
      owner = "alker0";
      repo = "chezmoi.vim";
      rev = "9b9f0479856f5ce61daa1d5889ff379b18a109a5";
      fetchSubmodules = false;
      sha256 = "sha256-Cti0Clg/rPbO+tEWfdY+q3cCAwnB3l7MFRteSmrfigQ=";
    });
    date = "2023-02-10";
  };
  k8s-snippets = {
    pname = "k8s-snippets";
    version = "c11e63d493c94a569533e82261ce474ef9ea9726";
    src = fetchFromGitHub ({
      owner = "budimanjojo";
      repo = "k8s-snippets";
      rev = "c11e63d493c94a569533e82261ce474ef9ea9726";
      fetchSubmodules = false;
      sha256 = "sha256-lFJMrrjqUGmpUTiuFzzObZMTPJVF9iqnWADUhpCFW00=";
    });
    date = "2023-02-10";
  };
  luasnip = {
    pname = "luasnip";
    version = "v1.2.1";
    src = fetchFromGitHub ({
      owner = "L3MON4D3";
      repo = "LuaSnip";
      rev = "v1.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-6NJPOu5f5ycu9l53CZB7prVNj61UaVz706tOrk5c860=";
    });
  };
  mason-lspconfig-nvim = {
    pname = "mason-lspconfig-nvim";
    version = "7a97a77eee486ae152d2c559a459eda7c8aa12aa";
    src = fetchFromGitHub ({
      owner = "williamboman";
      repo = "mason-lspconfig.nvim";
      rev = "7a97a77eee486ae152d2c559a459eda7c8aa12aa";
      fetchSubmodules = false;
      sha256 = "sha256-xBlMGlWfSeF2vBUZyNY/JeO+vY1MZAVv7WUMYyjUhvg=";
    });
    date = "2023-02-07";
  };
  mason-tool-installer-nvim = {
    pname = "mason-tool-installer-nvim";
    version = "6ca38efeb0406dea8da6c97f61d6f6ef30ab0576";
    src = fetchFromGitHub ({
      owner = "WhoIsSethDaniel";
      repo = "mason-tool-installer.nvim";
      rev = "6ca38efeb0406dea8da6c97f61d6f6ef30ab0576";
      fetchSubmodules = false;
      sha256 = "sha256-4PErcgZHuHPq3BHkXYFqpwRJlj/XZYdt8wqPUis8kCg=";
    });
    date = "2023-01-26";
  };
  oil-nvim = {
    pname = "oil-nvim";
    version = "7f176487052a155d43c6b64ef44b6dd775e94f99";
    src = fetchFromGitHub ({
      owner = "stevearc";
      repo = "oil.nvim";
      rev = "7f176487052a155d43c6b64ef44b6dd775e94f99";
      fetchSubmodules = false;
      sha256 = "sha256-Zw89rhpbzFOhOCffjClZuWoJuPcaMHAew9A3OORejDw=";
    });
    date = "2023-02-04";
  };
  sad-nvim = {
    pname = "sad-nvim";
    version = "1cdf3f25589dc92854913004443ba0e53bd103ad";
    src = fetchFromGitHub ({
      owner = "ray-x";
      repo = "sad.nvim";
      rev = "1cdf3f25589dc92854913004443ba0e53bd103ad";
      fetchSubmodules = false;
      sha256 = "sha256-6neCIpCEfxUwYdKZMsR3arILeuVMy6bfk/a5rSH1Wzo=";
    });
    date = "2023-01-30";
  };
  tokyonight-gtk-theme = {
    pname = "tokyonight-gtk-theme";
    version = "f7ae3421ac0d415ca57fb6224e093e12b8a980bb";
    src = fetchFromGitHub ({
      owner = "Fausto-Korpsvart";
      repo = "Tokyo-Night-GTK-Theme";
      rev = "f7ae3421ac0d415ca57fb6224e093e12b8a980bb";
      fetchSubmodules = false;
      sha256 = "sha256-90V55pRfgiaP1huhD+3456ziJ2EU24iNQHt5Ro+g+M0=";
    });
    date = "2023-01-17";
  };
  tokyonight-icon-theme = {
    pname = "tokyonight-icon-theme";
    version = "f7ae3421ac0d415ca57fb6224e093e12b8a980bb";
    src = fetchFromGitHub ({
      owner = "Fausto-Korpsvart";
      repo = "Tokyo-Night-GTK-Theme";
      rev = "f7ae3421ac0d415ca57fb6224e093e12b8a980bb";
      fetchSubmodules = false;
      sha256 = "sha256-90V55pRfgiaP1huhD+3456ziJ2EU24iNQHt5Ro+g+M0=";
    });
    date = "2023-01-17";
  };
}
